...
public class ElasticSearchLogStashRegexEventSerializer implements
    ...
    private Date appendBody(XContentBuilder builder, Event event)
        ...
        String category = headers.get("event-type");
...
//        logger.debug("regex = {}, fields = {}",regex,fields);
        if(storeOrgLog){
            builder.field("@message", new String(body, charset));
        }
...
//        logger.debug("Raw message = {}",raw);
        ...
        try {
            ...
            logger.debug("KV values = {}",vals.toString());
            ...
            for (String key : fieldIte) {
                ...
                if(key.equals(kvField) && !value.isEmpty()){    //该字段需按key：value方式解析
                    ...
                    if(kvs.length < kvNum){                     //如果分割出的key:value对数小于kvNum，则认为此kvField字符串无效，不再继续切分key value;
                        allFields.put(key.trim(), value.trim());
                        continue;
                    }
                    ...
                    for(String kvStr : kvs){
                        ...
                        if(kv.length == 2 && checkFieldNameIsValid(kv[0])){
                            allFields.put(kv[0].trim(), kv[1].trim());
                        }else{
                            extra += kvStr;
                        }
                    }
    ...
    private boolean checkFieldNameIsValid(String fieldName) {
        char firstChar = fieldName.trim().charAt(0);
    ...
    private void appendHeaders(XContentBuilder builder, Event event, Date date)
        ...
        String tag = headers.remove("tag");
        if (!StringUtils.isBlank(tag)) {
            ContentBuilderUtil.appendField(builder, "tag",
                    tag.getBytes(charset));
        }

        String application = headers.remove("application");
        if (!StringUtils.isBlank(application)) {
            ContentBuilderUtil.appendField(builder, "application",
                    application.getBytes(charset));
        }

        String category = headers.remove("category");
        if (!StringUtils.isBlank(category)) {
            ContentBuilderUtil.appendField(builder, "category",
                    category.getBytes(charset));
        }

        String environment = headers.remove("environment");
        if (!StringUtils.isBlank(environment)) {
            ContentBuilderUtil.appendField(builder, "environment",
                    environment.getBytes(charset));
        }
    ...
    public void configure(Context context) {
        ...
        for (String category : categoryArr) {
            ...
            if(regex != null && !regex.isEmpty()){
                cache.put(regex, Pattern.compile(regex,Pattern.DOTALL));
            }
        }
    }
